...
const mockWeather = (lat, lon) => {
  // Build sunrise/sunset as separate Date clones so they don't stomp on each
  // other — Date#setHours mutates in place and returns a timestamp, so reusing
  // one shared `now` object for both (as this used to do) is fragile and hard
  // to follow even though it happened to produce the right numbers.
  ...
  return {
    ...
    timestamp: new Date().toISOString(),   // Weather station ka last update time (mock)
